...
LP-MSPM0L1117 LaunchPad. It showcases PUBLISH frames (transmit), SUBSCRIBE
...
Visit [LP-MSPM0L1117](https://www.ti.com/tool/LP-MSPM0L1117) for LaunchPad information, including user guide and hardware files.

| Pin  | Peripheral | Function | LaunchPad Pin | LaunchPad Settings |
| ---- | ---------- | -------- | ------------- | ------------------ |
| PA18 | GPIOA      | PA18     | J3_26/J3_29   | <ul><li>PA18 can be connected to S1 button to VCC with external pull-down<br><ul><li>`J2 ON` Connect S1 button and external pull-down<br><li>`J2 OFF` Disconnect S1 button and external pull-down</ul><br><li>PA18 can be connected to XDS-110 BSL_Invoke<br><ul><li>`J22 17:18 OFF` Disconnect pin to XDS-110 BSL Invoke</ul><br><li>PA18 can be connected to boosterpack pin J3_29<br><ul><li>`J15 2:3` Connect pin to J3_29<br><li>`J15 1:2/OFF` Disconnect pin from J3_29</ul><br><li>PA18 can be connected to boosterpack pin J3_26<br><ul><li>`R58` is soldered and connects pin to J3_26 by default</ul></ul> |
| PB24 | GPIOB      | PB24     | J2_17         | N/A |
| PA5  | GPIOA      | PA5      | J6            | <ul><li>PA5 is connected to S2 button to GND with no external pull resistor<br><ul><li>Press `S2` button to connect pin to GND<br><li>Don't use `S2` button if not needed by application</ul></ul> |
| PB16 | GPIOB      | PB16     | J27_5         | <ul><li>PB16 can be connected to LED2 Blue<br><ul><li>`J5 ON` Connect to LED2 Blue<br><li>`J15 OFF` Disconnect from LED2 Blue</ul></ul> |
| PB17 | GPIOB      | PB17     | J27_8         | <ul><li>PB17 can be connected to LED2 Red<br><ul><li>`J4 ON` Connect to LED2 Red<br><li>`J4 OFF` Disconnect from LED2 Red</ul></ul> |
| PA11 | UART0      | RX       | J4_33/J22_8   | <ul><li>PA11 can be used as UART RX connected to XDS-110 backchannel, to boosterpack connector or to CAN/LIN connector:<br><ul><li>To use backchannel UART on J22:<br>  `J12 1:2`: Connects XDS-110 backchannel UART RX to `J22`<br>  `J22 7:8` Connects XDS-110 backchannel to UART RX<br><li>To use UART on boosterpack connector:<br>  `J12 2:3`: Connects UART RX to `J4_33`<br><li>To use on J26 CAN/LIN connector:<br>  `R63` is populated by default and connects pin to `J22_8`</ul></ul> |
| PA10 | UART0      | TX       | J4_34/J22_10  | <ul><li>PA10 can be used as UART TX connected to XDS-110 backchannel, to boosterpack connector or to CAN/LIN connector:<br><ul><li>To use backchannel UART on J22:<br>  `J10 1:2`: Connects XDS-110 backchannel UART TX to `J22`<br>  `J22 9:10` Connects XDS-110 backchannel to UART TX<br><li>To use UART on boosterpack connector:<br>  `J10 2:3`: Connects UART TX to `J4_34`<br><li>To use on J26 CAN/LIN connector:<br>  `R62` is populated by default and connects pin to `J22_10`</ul></ul> |
| PA20 | DEBUGSS    | SWCLK    | N/A           | <ul><li>PA20 is used by SWD during debugging<br><ul><li>`J22 15:16 ON` Connect to XDS-110 SWCLK while debugging<br><li>`J22 15:16 OFF` Disconnect from XDS-110 SWCLK if using pin in application</ul></ul> |
| PA19 | DEBUGSS    | SWDIO    | N/A           | <ul><li>PA19 is used by SWD during debugging<br><ul><li>`J22 13:14 ON` Connect to XDS-110 SWDIO while debugging<br><li>`J22 13:14 OFF` Disconnect from XDS-110 SWDIO if using pin in application</ul></ul> |
...
### Hardware Setup
Connect the LP-MSPM0L1117 LaunchPad to a LIN Responder or Network Analyzer via
